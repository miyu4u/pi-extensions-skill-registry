...
- `src/`
  ...
  - `indexing/`: corpus indexing, SQLite FTS5 persistence, search, relation expansion, diagnostics, decision, read packet, execution packet을 담당합니다.
    ...
    - `skill-scope-resolver.interface.ts`: scope-root 해석 계약을 정의합니다.
    - `skill-scope-resolver.service.ts`: scopeRoots/요청 scope를 scanner용 엔트리로 정규화·선택합니다.
    - `skill-file-scanner.ts`: filesystem에서 skill 문서 후보와 stat identity를 한 번에 수집합니다.
    - `source-manifest.interface.ts`: source scan 결과와 freshness signature contract를 정의합니다.
    - `source-manifest.service.ts`: path/size/mtime 기반 deterministic source signature를 생성합니다.
    ...
    - `results/`: `tool_result` compaction과 unknown skill read recovery의 service ownership을 소유합니다.
...
- unknown `skill://<name>` read의 `tool_result` compaction은 host `read` error 결과에서 `skill://` 대상, `Unknown skill`과 `Available signature`가 모두 match될 때만 적용합니다.
- `Available catalog` 원문 parse/copy는 하지 않으며, `tool_result` compaction은 recoverable signature만 좁게 전달합니다.
...
- unknown `skill://<name>` read compaction의 `isError: false` 전환은 host replacement 적용을 위한 의도된 transport trade-off이며, semantic unknown 상태를 숨기지 않습니다.
...
- 사용자가 특정 plan 문서의 최신화를 지시하면 실제 repository contract, 구현, tests, scripts를 대조하고 검증 시각과 구현 상태를 plan에 기록해야 합니다.
- `skill-registry`는 legacy `extensions` 경로로 직접 등록하지 않고 OMP `plugins` package로 설치·관리해야 합니다.
- unknown `skill://<name>` recovery는 host `read` resolver와 extension `resolve` ownership을 분리해야 합니다.
- direct unknown skill read compaction은 host source를 수정하지 않고 plugin `tool_result` hook에서 좁은 signature로 처리하며, host error replacement를 위해 `isError: false`로 전환되는 transport trade-off를 테스트와 문서에 명시해야 합니다.
- skill scope 기본 mapping은 `$cwd/**`를 `user-authored:local`, `$home/**`를 `user-authored:global`, `~/.omp/managed-skills/**`를 `managed-skills`로 분류하되, 해당 path와 scope 목록에 한정한 hardcode가 아니라 설정으로 확장·재정의할 수 있어야 합니다.
- `scope 미지정 조회는 모든 effective scope를 검색하고 configurable weighting을 적용하며, 기본 우선순위는 `user-authored:local > user-authored:global > managed-skills`로 유지해야 합니다.
- `scopes` 입력은 선택 입력이며 생략 시 모든 effective scope 탐색(omitted all-corpus mode), 명시되면 exact-matched scope만 탐색됩니다. 명시값이 빈 배열/`[""]` 또는 알 수 없는 scope만 포함하면 safe-zero를 반환해야 합니다.
- 결과/통계에서 `scope` 필드는 경로 분류값이고 `category`는 frontmatter 메타데이터로 독립적입니다.
- index cache metadata에는 `scopeRoots`/`scopePriority` 스냅샷을 보존하고, 복원/파싱 실패(특히 scope 관련 필드 포함) 시 캐시를 재생성해야 합니다.
- Filesystem source 삭제·추가·rename·stat 변경과 root 생성·삭제는 다음 `loadIndex()`에서 source manifest mismatch로 감지하며, active/SQLite snapshot은 signature가 일치할 때만 재사용해야 합니다.
- 커밋 정리는 파일 수가 아니라 독립적으로 설명하고 검증할 수 있는 의미 있는 변경 단위로 묶어야 합니다.
